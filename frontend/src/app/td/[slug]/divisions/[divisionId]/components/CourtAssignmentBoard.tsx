...
import type { MatchSummary, Bracket } from "@/hooks/useBrackets";
...
export function CourtAssignmentBoard({
  ...
  const busy =
    assignMatch.isPending ||
    rescheduleMatches.isPending ||
    retireMatches.isPending ||
    swapMatches.isPending ||
        ...
        <>
          <header className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant={data.queuePaused ? "destructive" : "default"} size="sm" onClick={handleQueueToggle} disabled={queuePause.isPending}>
                {queuePause.isPending ? "Updating…" : data.queuePaused ? "Resume queue" : "Pause queue"}
              </Button>
            </div>
          </header>
