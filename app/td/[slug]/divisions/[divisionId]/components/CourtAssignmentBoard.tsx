...
function ScoreEntryModal({ match, config, onCancel, onSubmit, isSubmitting }: ScoreEntryModalProps) {
  ...
  const adjustGameCount = (direction: "add" | "remove") => {
    setFormError(null);
    setGameCount((current) => {
      if (direction === "add") {
        return Math.min(maxGames, current + 1);
      }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    for (let index = 0; index < gameCount; index += 1) {
      ...
      const value1 = current.team1.trim();
      const value2 = current.team2.trim();

      if (value1 === "" || value2 === "") {
        setFormError("Enter scores for every game.");
        return;
      }

      const team1Score = Number(value1);
      const team2Score = Number(value2);

      if (!Number.isInteger(team1Score) || !Number.isInteger(team2Score)) {
        setFormError("Scores must be whole numbers.");
        return;
      }
      ...
      parsed.push({ team1: team1Score, team2: team2Score });
    }

    if (parsed.length === 0) {
      setFormError("Add at least one game before submitting the score.");
      return;
    }
                  ...
                  <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
                    <div className="flex flex-1 flex-col gap-1">
                      ...
                      <Input
                        ...
                        inputMode="numeric"
                        value={game.team1}
                        onChange={(event) => updateScore(index, "team1", event.target.value)}
                        min={0}
                      />
                    </div>
                    <div className="flex flex-1 flex-col gap-1">
                      ...
                      <Input
                        ...
                        inputMode="numeric"
                        value={game.team2}
                        onChange={(event) => updateScore(index, "team2", event.target.value)}
                        min={0}
                      />
                    </div>
                  </div>
  ...
  return (
    ...
    >
      <CardContent className="space-y-2 p-3">
        ...
        {match.startTime && (
          <p className="text-xs text-muted-foreground">
            Assigned {new Date(match.startTime).toLocaleTimeString()}
          </p>
        )}
        {match.priority !== undefined && (onMoveUp || onMoveDown) && (
          <p className="text-xs text-muted-foreground">Priority {match.priority}</p>
        )}
        {hasConflicts && (
...
export function CourtAssignmentBoard({
  ...
  const { data, isLoading, isError, error } = useBracketSchedule(
    slug,
    bracketId,
    token,
  );
  ...
  const queueMatches = useMemo(() => data?.queue ?? [], [data]);
  ...
  const handleCycleQueue = React.useCallback(async () => {
    ...
    const ids = queueMatches.map((match) => match.id);
  ...
  const openScoreEntry = React.useCallback(
    (match: MatchSummary) => {
      submitScore.reset();
      setFeedback(null);
      setErrorMessage(null);
  ...
  const openPrintSheet = React.useCallback(
    (match: MatchSummary) => {
  ...
  const handleSubmitScore = React.useCallback(
    async (games: Array<{ team1: number; team2: number }>) => {
      if (!scoreTarget) {
        throw new Error("No match selected for scoring.");
      }
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (!over || !active.data.current?.matchId) {
      return;
    }
    ...
    if (over.id === QUEUE_DROP_ID) {
      ...
      over.data?.current &&
      over.data.current.type === "court" &&
      typeof over.data.current.courtId === "string"
    ) {
      if (over.data.current.active === false) {
        setErrorMessage("Cannot assign to an inactive court.");
        return;
      }
      const isQueueMatch = queueMatches.some((match) => match.id === matchId);
    ...
    try {
      ...
      await assignMatch.mutateAsync({
        ...
        payload: {
          courtId,
          startTime: courtId ? new Date().toISOString() : null,
        },
      });
  ...
  const handleReschedule = async (match: MatchSummary) => {
    const next = window.prompt(
      "Enter new start time (ISO 8601) or leave blank to clear:",
      match.startTime ?? "",
    );
    ...
    try {
      setFeedback(null);
      setErrorMessage(null);
      await rescheduleMatches.mutateAsync({
        updates: [
          {
            matchId: match.id,
            startTime,
  ...
  const handleRetire = async (match: MatchSummary) => {
    if (!window.confirm("Retire this match?")) {
      return;
    }
    ...
    <section className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            variant={data.queuePaused ? "destructive" : "default"}
            size="sm"
            onClick={handleQueueToggle}
            disabled={queuePause.isLoading}
          >
            {queuePause.isLoading
              ? "Updating…"
              : data.queuePaused
            ...
            {data.queuePaused && (
              <Alert className="mt-3" variant="default">
                <AlertDescription className="text-xs">
                  Queue is paused. Resume to assign new matches.
                </AlertDescription>
              </Alert>
            )}
            <div className="mt-3 space-y-3">
              {queueMatches.length === 0 && (
                <p className="text-sm text-muted-foreground">
                  No matches waiting. Drag from courts to send a match back to the queue.
                </p>
              )}
